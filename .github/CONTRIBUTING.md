...
2️⃣ `npm run build` (must succeed — the published tarball is a single-file `dist/index.js` produced by `tsup`)
...
6️⃣ If you add or rename a tool, update the 🛠️ `Tools` section in the README and the entry in `src/tools.ts` (`toolDefinitions` array) — including the correct `scopes` and the `readOnlyHint` / `destructiveHint` / `idempotentHint` annotations

7️⃣ New file-path inputs must be routed through `resolveDownloadSavePath()` or `assertAttachmentPathAllowed()` (no raw `fs.writeFileSync(userPath, …)` — see `src/utl.ts` for the canonical jail helpers)
