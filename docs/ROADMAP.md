...
- **v1.0.0 on npm** — cut once the three items above land. Every release is already signed with Sigstore (keyless GitHub OIDC), ships an SLSA in-toto attestation, and carries npm provenance — the `0.x` line on npm has the exact same supply-chain posture, the `1.0.0` cut is purely a maturity / API-stability signal.
