...
- **MCP `outputSchema` per tool** — once the migration to `McpServer` (below) lands, extend `defineTool()` (the wrapper that will replace today's monolithic `CallToolRequestSchema` switch) with an optional `outputSchema?: ZodRawShape` and write a Zod schema for each of the ~26 tools. The MCP spec (2025-06-18+) supports `outputSchema` so clients can validate `structuredContent` and the LLM gets typed return shapes. Lets us drop the textual `RETURNS:` block from tool descriptions and rely on a machine-readable contract instead. Naturally folds into the migration item below — same time, same touch points.
