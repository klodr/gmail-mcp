...
- **`nodemailer` bumped `8.0.5 → 8.0.6`** — patch release on the attachment-aware MIME assembly path used by `send_email` / `draft_email` / `reply_all`. No CVE in 8.0.5, the bump is hygiene before the v1.0.0 cut.
- **`src/oauth-flow.ts` — hard 64 KB size cap on `gcp-oauth.keys.json` and `credentials.json`** (security audit defence-in-depth + CodeQL TOCTOU follow-up). Both files are normally 1-3 KB; the cap is ~30× the realistic upper bound but prevents an accidentally-corrupted (partial write, disk corruption) or hostile multi-GB JSON from being read into memory + `JSON.parse`-d at boot. New `readJsonBounded(absPath)` helper opens the file once and runs `fstat` + `read` on the SAME file descriptor (CodeQL flagged a `statSync` → `readFileSync` race window where a concurrent writer could grow the file in the gap; same-fd closes that). The error message branches on `path.basename(absPath)` so the operator gets the right remediation path: `gcp-oauth.keys.json` → "Replace from Google Cloud Console", `credentials.json` → "Re-run `npx @klodr/gmail-mcp auth` to regenerate". 4 new unit tests in `src/oauth-flow.test.ts` cover the happy path + each remediation branch.
