...
- **`src/oauth-flow.ts` — hard 64 KB size cap on `gcp-oauth.keys.json` and `credentials.json`** (security audit defence-in-depth). Both files are normally 1-3 KB; the cap is ~30× the realistic upper bound but prevents an accidentally-corrupted (partial write, disk corruption) or hostile multi-GB JSON from being read into memory + `JSON.parse`-d at boot. New `readJsonBounded(absPath)` helper does `fs.statSync` first and throws a clear "exceeds the 64 KB cap" error when the file is too large, surfacing the boot-time misconfiguration without loading the payload.
