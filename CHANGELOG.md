...
- **`reply_to_email` tool** — first-class sender-only reply. The handler fetches the source message, picks the original `From:` mailbox as the sole recipient (no Cc broadcast), preserves `Subject:` with a `Re:` prefix, and sets `In-Reply-To` / `References` automatically before routing through `sendOrDraftEmail`. Bails with `isError` when the source `From:` is empty so the agent never silently sends to nothing. Inherits the `GMAIL_MCP_RECIPIENT_PAIRING` gate, audit-log elision, dry-run, and 60-second timeout from the existing send pipeline. Closes the gap between the broadcast-y `reply_all` and the manual `send_email` + `In-Reply-To` recipe documented in the ROADMAP.
