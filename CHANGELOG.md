...
- **`reply_to_email` tool** — first-class sender-only reply. The handler fetches the source message and picks the reply destination per RFC 5322 §3.6.2 precedence: **`Reply-To:`** if present (the mailing-list pattern: From=list, Reply-To=author — replying to From would broadcast a private reply to the whole list), otherwise **`Sender:`** when the source has a single Sender mailbox without Reply-To, otherwise **`From:`** when there is exactly one mailbox. Multiple From mailboxes with no Reply-To/Sender disambiguator → `isError` so the agent disambiguates explicitly. Preserves `Subject:` with a `Re:` prefix and sets `In-Reply-To` / `References` automatically before routing through `sendOrDraftEmail`. Inherits the `GMAIL_MCP_RECIPIENT_PAIRING` gate, audit-log elision, dry-run, and 60-second timeout from the existing send pipeline. Closes the gap between the broadcast-y `reply_all` and the manual `send_email` + `In-Reply-To` recipe documented in the ROADMAP.
