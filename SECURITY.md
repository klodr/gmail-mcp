...
- **Enable the audit log in production.** Set `GMAIL_MCP_AUDIT_LOG=/abs/path/to/audit.jsonl` to capture a tamper-evident JSONL trail of every tool call, its redacted arguments, and its terminal state (`ok` / `error` / `rate_limited`). Counterparty PII (subject, body, to/cc/bcc, snippet) is elided by default; flip `GMAIL_MCP_AUDIT_LOG_VERBOSE=true` only on isolated dev machines where the log will not land in a SIEM.
