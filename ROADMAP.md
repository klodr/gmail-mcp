...
- **Multi-account support** — one MCP instance juggling several Gmail accounts. New `auth <account-name>` command, `credentials-<name>.json` per account, optional `account` arg on every tool, `list_accounts` tool. Must stay fully backwards-compatible with single-account setups (default account when `account` is omitted). Fold into the `McpServer` migration so the handler-extraction pass carries the account parameter uniformly instead of touching 25 tool cases a second time.
